...
'''
NEW THIS VERSION
-runs from command line $python anabatic_flow_v5.py
...
class Integrator():
    ...
    def deriv_dfsn_only(self, u_old, u_old2, theta_old, theta_old2):
        ...
        #compute derivative 
        A = Km*dt/dn**2         #alpha for FCTS u
        B = Kh*dt/dn**2         #alpha for FCTS theta
        C = 2*Km*dt/dn          #alpha for DFF u
        D = 2*Kh*dt/dn          #alpha for DFF theta
        
        for n in np.arange(1,len(u_new) -1, 1):
            #FTCS (7.9) -- unstable for practical size timesteps!
            #u_new[n] = (1 - A)*u_old[n] + A*(u_old[n-1] + u_old[n+1])
            #theta_new[n] = (1 - B)*theta_old[n] + B*(theta_old[n-1] + theta_old[n+1])
    ...
    def deriv_DFF(self, u_old, u_old2, theta_old, theta_old2):
        '''
        Calculates t(j+1)th element of (1) and (2) using the modified 
        DuFort-Frankel discretization described in the paper
        
        uses convention new, old, old2 for t(j+1), t(j), t(j-1)
        '''
        #define constants
        dt = self.timevars.dt                  #s
        dn = self.nvars.dn                     #m
        ...
        #group constants into coeffs A, B, C for velocity
        #                        and P, Q, R for theta
        A = (N2 / gamma) * np.sin((np.pi/180) * beta)
        B = 1 / (2*dt)
        C = Km / dn**2
        
        P = 1 / (2*dt)
        Q = gamma * np.sin((np.pi/180) * beta)
        R = Kh / dn**2
        ...
        for n in np.arange(1,len(u_new) -1, 1):
            ...
            u_new[n] = (A*theta_old2[n] + B*u_old2[n] + C*(u_old[n+1] - u_old2[n] + u_old[n-1])) / (B + C)
            theta_new[n] = (P*theta_old2[n] - Q*u_old2[n] + R*(theta_old[n+1] - theta_old2[n] + theta_old[n-1])) / (P - R)
...
if __name__ == '__main__':
    anabatic_run = Integrator('anabatic_lowres.yaml')
