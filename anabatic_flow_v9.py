...
'''
Function Library for anabatic_flow_main ver 9
----------------------------------------
...
Introduced "clouds mode" and "fire mode" in boundary conditions, two ridiculously simple
models that introduce new forcing at the surface

Experimental 'storm' forcing that makes the wind -10m/s for a third of the sim
...
class Integrator():
    ...
    def apply_bc(self, u_grid, theta_grid, tstep, forcing='const'):
        ...
        elif forcing == 'diurnal':
            #note diurnal forcing will scale tend to be 24h ie oscillate 1 period 
            theta_grid[tstep][0] = boundaryconds.theta_surf * np.sin(omega*tstep) 
        
        elif forcing == 'clouds':
            #square wave forcing, fair weather cumulus clouds passing over the sun periodically
            if np.sin(3*omega*tstep) > 0.5:
                theta_grid[tstep][0] = 8.
            else:
                theta_grid[tstep][0] = 2.
       
        elif forcing == 'storm':
            #for the middle third of the sim, a gust front forces the wind in the opposite 
            #(downslope) direction at 5m/s
            storm_start = ((timevars.tend / timevars.dt) // 3)
            storm_stop = 2 * ((timevars.tend / timevars.dt) // 3)
            
            if storm_start < tstep < storm_stop:
                u_grid[tstep][1:-1] = -10 #m/s
                theta_grid[tstep][0] = 2.
            else:
                theta_grid[tstep][0] = 5.
    
        elif forcing == 'fire':
            #Sets the slope on fire halfway through the simulation. 
            #Surface temperature goes to 500 degrees!
            if tstep < ((timevars.tend / timevars.dt) // 2 ):
                theta_grid[tstep][0] = boundaryconds.theta_surf
            else:
                theta_grid[tstep][0] = 500. #K
